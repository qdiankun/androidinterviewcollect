 ...
 * <p/>
 * 问题：旋转屏幕后重新创建了Activity,导致之前请求到的数据丢失
 * 解决方法一：不让Activity重新创建
 * ① 不许屏幕旋转    android:screenOrientation="portrait"
 * ② 设置旋转屏幕后不重新创建  android:configChanges="orientation|keyboardHidden|screenSize"
 * 解决方法二：重新创建Activity 恢复之前的数据
 * ① onSaveInstanceState() 销毁前保存数据   ，在onCreate(Bundle savedInstanceState)恢复数据
 * ② 使用Fragment来处理Configuration Change 参考 ConfigChangeUseFragActivity类
 */
...
//        }
    ...
    @OnClick(R.id.btn_test_destroy)
    void testDestroy() {
        openActivity(ConfigFinalActivity.class);
    }
    @OnClick(R.id.btn_test_two)
    void testRotain() {
        openActivity(ConfigFinalTwoActivity.class);
    }
